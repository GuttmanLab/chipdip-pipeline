...
TRIM_RD = expand([out_dir + "workup/trimmed/{sample}_R1.part_{splitid}.barcoded_dpm.RDtrim.fastq.gz", out_dir + "workup/trimmed/{sample}_R1.part_{splitid}.barcoded_bpm.RDtrim.fastq.gz"], sample = ALL_SAMPLES, splitid=NUM_CHUNKS)
...
SPLIT_DPM_BPM = expand([out_dir + "workup/fastqs/{sample}_R1.part_{splitid}.barcoded_bpm.fastq.gz", out_dir + "workup/fastqs/{sample}_R1.part_{splitid}.barcoded_dpm.fastq.gz"], sample=ALL_SAMPLES, splitid=NUM_CHUNKS)

################################################################################
#DNA workup
################################################################################

Bt2_DNA_ALIGN = expand(out_dir + "workup/alignments_parts/{sample}.part_{splitid}.DNA.bowtie2.mapq20.bam",sample=ALL_SAMPLES, splitid=NUM_CHUNKS)

MERGE_DNA = expand(out_dir + "workup/alignments/{sample}.DNA.merged.bam", sample = ALL_SAMPLES)

CHR_DNA = expand(out_dir + "workup/alignments_parts/{sample}.part_{splitid}.DNA.chr.bam", sample=ALL_SAMPLES, splitid=NUM_CHUNKS)

MASKED = expand(out_dir + "workup/alignments_parts/{sample}.part_{splitid}.DNA.chr.masked.bam", sample=ALL_SAMPLES, splitid=NUM_CHUNKS)

################################################################################
#Bead workup
################################################################################

FQ_TO_BAM = expand(out_dir + "workup/alignments_parts/{sample}.part_{splitid}.BPM.bam", sample=ALL_SAMPLES, splitid=NUM_CHUNKS)
...
CLUSTERS = expand(out_dir + "workup/clusters_parts/{sample}.part_{splitid}.clusters", sample=ALL_SAMPLES, splitid=NUM_CHUNKS)
...
SPLITBAMS = expand(out_dir + "workup/alignments/{sample}.DNA.merged.labeled.bam", sample=ALL_SAMPLES)
...
rule splitfq:
    ...
    output:
        expand([temp(out_dir + "workup/splitfq/{{sample}}_R1.part_{splitid}.fastq"), temp(out_dir + "workup/splitfq/{{sample}}_R2.part_{splitid}.fastq")],  splitid=NUM_CHUNKS)
    params:
...
rule merge_dna:
    input:
        expand(out_dir + "workup/alignments_parts/{{sample}}.part_{splitid}.DNA.chr.masked.bam", splitid=NUM_CHUNKS)
    output:
...
rule merge_beads:
    input:
        expand(out_dir + "workup/alignments_parts/{{sample}}.part_{splitid}.BPM.bam", splitid = NUM_CHUNKS)
    output:
...
rule merge_clusters:
    input:
        expand(out_dir + "workup/clusters_parts/{{sample}}.part_{splitid}.clusters", splitid=NUM_CHUNKS)
    output:
...
rule thresh_and_split:
    ...
    shell:
        '''
        (python {tag_and_split} -i {input.bam} -c {input.clusters} -o {output.bam} -d workup/splitbams --min_oligos {min_oligos} --proportion {proportion} --max_size {max_size} --num_tags {num_tags}) &> {log}
        '''
