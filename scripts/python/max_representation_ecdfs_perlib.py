...
def main():
    ...
    ecdf_plot_fig.savefig(args.directory + '/Max_representation_ecdf.pdf', bbox_inches='tight')
    ecdf_counts_ax = 'None'
    for f in files:
        ecdf_counts_ax = max_representation_ecdf_counts(f, ecdf_counts_ax, args.xlim)
    ecdf_counts_fig =  ecdf_counts_ax.get_figure()
    ecdf_counts_fig.savefig(args.directory + '/Max_representation_counts.pdf', bbox_inches='tight')
...
def max_representation_ecdf(clusterfile, ax):
    ...
    ax.set(xlabel = 'Maximum Bead Representation Proportion', ylabel='Proportion of Beads')
    ax.legend()
    return ax
